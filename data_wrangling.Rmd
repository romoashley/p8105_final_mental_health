...
I converted dates to the date format using `mdy` and created a new variable named `year` and convert it to a numeric variable. I separated the original `time_period_label` variable into two variables: `week_label` and `years` and deleted `years` (because this variable is unable convert into numeric without causing missing value problem and we already had a `year` variable).
