...
## State-level
To look at the prevalence of each indicator of mental health care utilization by state, I filtered the main dataset to only include observations at the state-level. For subsequent analyses, I used filter to select only observations for the indicator of interest. Next, I grouped by state to calculate the mean prevalence of each indicator by state, then created a table of the top 5 states by mean prevalence between 2020-2022 for each indicator. Then, I created a heat map showing mean prevalence by state for each indicator between 2020-2022 using the sf and maps packages. Next, I grouped by state and year to calculate the mean prevalence by state and year and then calculate the change in mean prevalence between 2020-2022 by state. Using this data, I created a table of the 5 states with the highest change in mean prevalence between 2020-2022 for each indicator. Lastly, I created an interactive graph for each indicator showing change in mean prevalence for each state over time using plotly. The result is 16 graphs or tables that describe state-level trends in mental health care utilization and access in the US between 2020-2022.

## Additional Analysis

We conducted a [independent samples t-tests and ANOVA](statistical_analysis.html) to determine if there were statistically significant differences in access to mental health for each demographic characteristic.
